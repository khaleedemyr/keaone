...
import { findProductByScan, resolveProductFromScan } from './purchaseLineUtils'
...
function CameraScanButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <button
      ...
      title={t('procurementScanCamera')}
      aria-label={t('procurementScanCamera')}
      onClick={onClick}
    >
...
export function ProcurementQuickAddBar({
  productOptions,
  products,
  onPick,
}: {
  ...
  useEffect(() => {
    ...
    function onDoc(event: MouseEvent) {
      ...
      if ((event.target as HTMLElement | null)?.closest?.('[data-procurement-quick-add-menu]')) return
      setOpen(false)
    }
  ...
  function pickFromCode(code: string) {
    ...
    if (product) {
      pick(String(product.id))
      return true
    }

    feedback.error(t('procurementScanNotFound', { code: trimmed }))
    return false
  }
  ...
  return (
    <>
      ...
      {showMenu && typeof document !== 'undefined'
        ? createPortal(
            <div
              data-procurement-quick-add-menu
              id={listId}
              role="listbox"
