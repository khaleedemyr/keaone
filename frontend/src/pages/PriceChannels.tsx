...
export default function PriceChannels() {
  ...
  const [items, setItems] = useState<PriceChannel[]>([])
  const [name, setName] = useState('')
  const [editing, setEditing] = useState<PriceChannel | null>(null)
  const [viewing, setViewing] = useState<PriceChannel | null>(null)
  ...
  async function load() {
    try {
      const { data } = await api.get<ApiOk<PriceChannel[]>>('/price-channels', {
        params: {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    setError('')
    try {
      if (editing) await api.put(`/price-channels/${editing.id}`, { name })
      else await api.post('/price-channels', { name })
      setOpen(false)
      await load()
  ...
  async function remove(item: PriceChannel) {
    const ok = await feedback.confirm({
      title: t('deletePriceChannelTitle'),
    ...
    if (!ok) return
    try {
      await api.delete(`/price-channels/${item.id}`)
      await load()
  ...
  async function activate(item: PriceChannel) {
    try {
      await api.put(`/price-channels/${item.id}`, { is_active: true })
      await load()
      feedback.success(t('saved'))
    } catch (err) {
  ...
  const canEdit = can('pricechannels', 'edit')
  const canDelete = can('pricechannels', 'delete')
  ...
  return (
    <div>
      <PageHeader
        ...
        title={t('navPriceChannels')}
        subtitle={t('priceChannelsSubtitle')}
        action={
          can('pricechannels', 'create') ? (
      ...
      <MasterFilters {...list.filters} searchPlaceholder={t('searchPriceChannel')} />
      ...
      <MasterModal
        open={open}
        title={editing ? t('editPriceChannel') : t('newPriceChannel')}
        error={error}
        saving={saving}
