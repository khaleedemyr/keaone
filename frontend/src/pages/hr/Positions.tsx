...
import type { ApiOk, Position } from '../../types'
...
export default function Positions() {
  ...
  async function load() {
    try {
      const { data } = await api.get<ApiOk<Position[]>>('/positions', {
        params: {
  ...
  function openEdit(item: Position) {
    ...
    setError('')
    setOpen(true)
    logMasterForm('position', 'edit', item.name)
  }
  ...
  async function activate(item: Position) {
    try {
      await api.put(`/positions/${item.id}`, { is_active: true })
      await load()
      feedback.success(t('saved'))
    } catch (err) {
  ...
  return (
    <div>
      ...
      <MasterFilters {...list.filters} searchPlaceholder={t('searchPosition')} />
      ...
      <MasterModal
        open={open}
        title={editing ? t('editPosition') : t('newPosition')}
        error={error}
        saving={saving}
      ...
      >
        ...
        <label className="text-sm text-muted">
          ...
          <span className="mt-1 block text-xs">{t('positionRankHint')}</span>
        </label>
        <label className="text-sm text-muted">
          {t('sortOrder')}
          <input
